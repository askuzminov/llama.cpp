@echo off
rem per-op timings from the vulkan backend at a deep context, one run per value of
rem GGML_VK_FA_SPARSE in FAVARIANTS. produces a lot of output, keep the run short. cuda ignores
rem GGML_VK_PERF_LOGGER.
rem the point of the sweep: the gather path walks the list of cells a query tile can see, the
rem dense path walks all of KV but keeps the aligned kernel. the FLASH_ATTN_EXT line says which
