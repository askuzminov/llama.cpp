...
rem each value of FAVARIANTS is one arm: 1 sparse gather, 0 dense, g gather with the rows of the
rem tile unioned into one list. the union is exact, so g has to land on the same number. 06 tells
rem how far the two kernels sit from each other, this one tells how far each sits from the
...
rem %1 = 1 gather on, 0 gather off, g gather over the union of the rows of the tile
:run
