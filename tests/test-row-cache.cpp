...
// dequantizing path, and the budget is kept small enough that every gather has to evict.
...
static void test_type(ggml_type type, int64_t n_cols, int64_t n_rows, size_t budget, int n_threads) {
    const std::string path = write_table("test-row-cache.bin", type, n_cols, n_rows);

    {
        llama_row_cache cache(path, HEADER, type, n_cols, n_rows, budget, n_threads);

        check(cache.budget() >= cache.block_size(), "budget holds at least one block");
        check(cache.budget() % cache.block_size() == 0, "budget is whole blocks");
...
int main() {
    ...
    for (const char * run : { "1", "64" }) {
        ...
        test_type(GGML_TYPE_F32, 128, 1000, 1024*1024, 4);

        // rows longer than a block, so every row straddles block boundaries
        test_type(GGML_TYPE_F32, 4096, 300, 2*1024*1024, 2);

        // dequantization, with a row size that is not a power of two
        test_type(GGML_TYPE_Q8_0, 1536, 500, 1024*1024, 3);

        // a budget below the minimum: the cache raises it instead of failing
        test_type(GGML_TYPE_F32, 2048, 100, 1024, 1);

        // reader count picked by the cache itself
        test_type(GGML_TYPE_Q8_0, 4096, 400, 4*1024*1024, 0);
    }
