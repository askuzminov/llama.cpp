...
struct llama_model_params common_model_params_to_llama(common_params & params) {
    ...
    mparams.lazy_cache_mib = params.lazy_cache_mib;
...
size_t common_host_mem_available() {
    ...
    // free + inactive + purgeable pages are effectively reclaimable
    const uint64_t avail_pages = (uint64_t) vm.free_count + vm.inactive_count + vm.purgeable_count;
    return (size_t) (avail_pages * (uint64_t) page_size);
#elif defined(__linux__)
