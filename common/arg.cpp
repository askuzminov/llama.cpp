...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        {"--ctx-checkpoints-tree"},
        "[experimental] keep context checkpoints as a branch tree (enables reuse of multiple divergent branches, useful for agents) instead of a linear chain (default: disabled)",
        [](common_params & params) {
            params.ctx_checkpoints_tree = true;
        }
    ).set_env("LLAMA_ARG_CTX_CHECKPOINTS_TREE").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI}));
    add_opt(common_arg(
