...
- Одну папку спилла можно отдать нескольким моделям. Имя файла - `state-<сигнатура>-<uid>.bin`,
  где сигнатура - та самая, по которой файл принимается при старте (модель плюс конфигурация KV).
  Раньше имя было `state-<uid>.bin`, а счётчик `uid` у каждой модели свой, поэтому модели писали в
  одни и те же имена; `restore_spilled` при этом удаляла всё, что не подошло по сигнатуре. В режиме
  роутера с `--models-max 1` модели сменяют друг друга, и каждая смена стирала кэш предыдущей:
  проверено на двух моделях в общей папке - файл первой (1150976 Б) после загрузки второй
  превращался в её 16384 Б, и возврат к первой давал `cache_n = 0` на том же промпте в 1742 токена.
  Теперь чужие файлы не читаются и не удаляются, возврат к первой модели даёт `prompt_n = 1,
  cache_n = 1741`. Файлы старого образца удаляются один раз при старте - их всё равно нельзя
  отнести к модели. Бюджет `--cache-disk` считает свои файлы, то есть он на модель: общая папка
  вмещает по N МиБ каждой.
- Буферы состояния выделяются выровненными на 4096 (`server_state_alloc` в
  ...
  Через буфер идут только заголовок, сериализованный промпт и хвост короче блока. На NVMe с обходом
  кэша ОС на 512 МиБ полезной нагрузки: запись 3.15 против 2.10 ГБ/с, чтение 3.10 против 1.86 ГБ/с.
  Ускорение не потребовало менять формат файла.
- При старте читаются только заголовок и промпт каждого файла, полезная нагрузка - лениво,
...
- Промпты с картинками тоже пишутся на диск. Раньше файл хранил голый массив `llama_token`, а
  медиа-чанк в списке токенов представлен только заполнителями `LLAMA_TOKEN_NULL`, поэтому такие
  промпты в спилл не попадали: они жили в RAM, терялись при перезапуске и при нехватке памяти
  выбрасывались вместо сброса на диск. Теперь секция промпта - это `server_tokens::serialize()`
  (тот же формат, что у сохранения слотов), он несёт и чанки. Пиксели туда не идут:
  `clip_image_f32::serialize` не пишет `buf`, а картинка и так уже закодирована в блобе состояния,
  для сопоставления префикса нужны только id чанка (sha256 картинки, то есть стабилен между
  запусками) и число токенов. Замер на `tinygemma3`: 274 токена с одной картинкой - 1260 байт
  секции промпта при 4.29 МиБ блоба. `SPILL_VERSION` поднят с 2 до 3, файлы прежней версии
  удаляются при старте.
- Остановка на Windows больше не теряет кэш. Обработчик консоли в
  [tools/server/server.cpp](tools/server/server.cpp) отвечал только на `CTRL_C_EVENT`, а на
  закрытие окна, выход из системы и выключение ПК возвращал `FALSE` - и Windows убивала процесс,
  не дав записать кэш. Теперь эти события обрабатываются. Возврат из обработчика сам по себе
  завершает процесс, поэтому обработчик ждёт: флаг `has_exited` ставит `exit_notifier`, объявленный
  до `ctx_server`, то есть уничтожаемый после него - а кэш пишет как раз `~server_context`.
  `CTRL_BREAK_EVENT` теперь тоже гасит сервер аккуратно. Дочерний процесс роутера эти три
  события пропускает: его гасит роутер командой в stdin, а поток, который эту команду ждёт,
  джойнится раньше, чем пишется кэш - собственный выход по событию консоли там бы повис.
- Две границы времени на этот путь. `HungAppTimeout` в `HKCU\Control Panel\Desktop` (5 с по
  умолчанию) - сколько Windows ждёт при закрытии окна и выключении ПК; у Ctrl+C такого предела нет.
  `stop-timeout` в `models.ini` (10 с по умолчанию) - сколько роутер ждёт дочерний процесс с
  моделью, прежде чем убить его. В режиме роутера кэш принадлежит дочернему процессу, и путь такой:
  роутер получает событие -> `unload_all()` шлёт каждому потомку команду выхода в stdin -> потомок
  отдаёт слоты кэшу и дописывает недостающее. На практике ждать почти нечего: запись с опережением
  уже вынесла состояния на диск, пока слоты простаивали. `taskkill /F` этот путь не проходит.
...
- Замер объединения на Strix Halo, 21.09. 04, `pp2048`, объединение против тайла в одну строку:
  на d32768 362.63 против 284.57 т/с при `-ub 512` и 369.40 против 296.18 т/с при `-ub 2048`,
  на d65536 312.70 против 255.52 и 301.06 против 263.10, на d122880 274.26 против 234.00 и
  258.67 против 238.05. Против плотного ядра это в 1.16-1.38 раза на d32768, в 1.55-2.19 на
  d65536 и в 2.56-3.01 на d122880. 07, `pp512 @ d64000`, `-ub 2048`: узел flash attention
  44.4 мс против 73.4 мс, граф 1.55 с против 1.90 с. Качество не двигается: 05 даёт одну и ту же
  PPL 4.2435 +/- 0.03626 на всех трёх разреженных плечах, 06 - один и тот же KLD 0.012960 до
  ...
  не дотягивал 1.8% до 65664: сборка брала форму, которая на 24% медленнее, а на d32768 - на 28%.
  Худший случай
