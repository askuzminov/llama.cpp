...
- Проекция `hc_*_inject` считается с переставленными операндами. Она даёт всего `hc = 4` строки
  ...
  стоит несколько КиБ. `test-backend-ops perf` на этой форме: 8751.68 мкс против 984.50 мкс,
  то есть 8.9x; транспонирование - 21.43 мкс на той же машине. Лоры для этого тензора больше нет,
  веса hyper-connections не бывают целью адаптера.
...
- Замер на Strix Halo, Qwen3.8-Flash-Next Q4_K_XL, `pp2048 @ d65536`: 223.88 против 181.82 т/с
  ...
  Узел flash attention занимает 74.3 мс против 118.8 мс, весь граф 2.12 с против 2.66 с.
...
- `GGML_VK_MMID_WG256=1` поднимает средний и большой тайлы `mul_mat_id` до 256 потоков на AMD с
  coopmat: плотный большой тайл уже считает 128x128 на 256 потоках, а варианты `mul_mat_id`
  оставались на 128. Те же BM/BN/BK, то есть та же разделяемая память, вдвое больше потоков на
  загрузку тайла A/B, вдвое меньше аккумуляторов на поток. Портировано из
  `halo-box/strix-llama.cpp` `f4d6f9e8d`.
- `GGML_VK_MMID_SCALE_EPILOGUE=1` применяет следующий за `MUL_MAT_ID` узел `MUL`
  (`ffn_moe_weighted`) прямо при записи результата матмула, убирая запись и обратное чтение всего
  результата. В апстриме этот фьюз включён только для мат-вектора, то есть работает на декоде и не
  работает на префиле. Отказ на coopmat2: там эпилога в шейдере нет. Оттуда же, `f4d6f9e8d`.
