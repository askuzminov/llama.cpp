...
- Выбор между ними сборка делает сама: сначала объединение, если оно окупается, иначе тайл в
  одну строку, иначе плотное ядро. Объединение окупается при `KV >= min_ratio * Br * n_kv_max`,
  где `min_ratio` это 4 на coopmat2 и 2 в остальных случаях. Размер тайла в пороге обязателен:
  список объединения длиной `Br * n_kv_max`, и без множителя порог пропускает случаи, где ёмкость
  упирается в KV - обход тогда не короче плотного, а пре-пасс и индирекция уже оплачены.
  На Strix Halo (coopmat1, `Br = 16`, бюджет 2052) это `KV >= 65664`, то есть объединение
  включается на глубинах от 64K; на coopmat2 тайл выше (`Br = 64`) и `min_ratio` равен 4, порог
  `KV >= 525312` не достигается. `GGML_VK_FA_SPARSE_GROUP` двигает этот порог (`0` - никогда,
  `1` - как только список короче KV), `GGML_VK_FA_SPARSE_ROW_RATIO` - порог тайла в одну строку
  (по умолчанию 8). Плечи `g` и `r` в 04, 05, 06 и 07 меряют обе формы по отдельности.
