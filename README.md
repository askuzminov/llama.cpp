...
- Драфты слотов выравниваются по длине ([common/speculative.cpp](common/speculative.cpp)): слот,
  упёршийся в `p-min`, продолжает драфтить, пока драфтит хоть один другой, а в конце все обрезаются
  до самой длинной «естественной» длины. Добитые позиции - настоящие предсказания MTP-головы, просто
  ниже порога, и голова за них почти не платит: этот шаг она и так декодирует для второго слота.
  Включается только при `draft-mtp`, больше чем одном драфтящем слоте и рекуррентной или гибридной
  цели; при `-np 1` поведение не меняется. Цена - лишние позиции в проходе, и она пока не измерена,
  методика в [scripts/win-qsa/README.md](scripts/win-qsa/README.md).
