...
- Таблица PLE (`per_layer_token_embd.weight`, 26.82 GiB у `qwen4exp`) может остаться на диске:
  ...
  занимают 0.25 GiB. Пул по умолчанию 8 МиБ: больше не окупается, а `-lzm dio:0` убирает его совсем -
  тогда в пуле живёт только тот кусок, который читается прямо сейчас, и перед файлом остаётся один
  страничный кеш системы. Чтение буферизованное, поэтому страничный кеш держит выданные строки в любой
...
- Замер на Strix Halo, llama-perplexity, 16 чанков wikitext по 32768, тёплый страничный кеш:
  `-lzm on` 259.35 т/с префила, `-lzm dio` 343.27 т/с, PPL 3.9400 в обоих. Бюджет пула на это не
  влияет - 8 МиБ дают 342.60 т/с, 1024 МиБ 343.05 т/с, ожидание чтений 3.5-4.0 с из 382 с прохода
  при любом размере, потому что те же страницы держит файловый кеш системы. Поэтому умолчание
  пула - 8 МиБ. Небуферизованное чтение той же таблицы давало 38.57 s/pass: на этом файле оно не
