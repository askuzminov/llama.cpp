...
| `--ctx-checkpoints-tree` | [experimental] keep context checkpoints as a branch tree (enables reuse of multiple divergent branches, useful for agents) instead of a linear chain (default: disabled)<br/>(env: LLAMA_ARG_CTX_CHECKPOINTS_TREE) |
...
| `--cache-spill-dir PATH` | [experimental] spill cold prompt-cache states to PATH (on disk) instead of dropping them under memory pressure; loaded back on a cache hit. The files are kept on shutdown and picked up again on the next start (cold-start reuse) as long as the model and KV configuration match. Best on fast NVMe. (default: disabled)<br/>(env: LLAMA_ARG_CACHE_SPILL_DIR) |
| `--cache-disk N` | [experimental] disk budget in MiB for spilled prompt-cache states (see --cache-spill-dir); the least frequently used states are dropped once exceeded (default: 0, 0 = unlimited)<br/>(env: LLAMA_ARG_CACHE_DISK) |
