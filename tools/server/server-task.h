...
struct server_prompt {
    ...
    // [experimental] branch-tree checkpoints (used when --ctx-checkpoints-tree is set).
    // checkpoint_cur is the tree node the current live sequence descends from (-1 = none).
    common_checkpoint_tree checkpoint_tree;
    int32_t checkpoint_cur = -1;
...
struct server_prompt_cache_state {
    ...
    // RAM-resident bytes. A spilled state contributes 0 for `data` (freed), only its (small)
    // checkpoint metadata remains resident.
    size_t size() const {
        ...
        res += prompt.checkpoint_tree.size_bytes();
...
struct server_prompt_cache {
    server_prompt_cache(int32_t limit_size_mib, size_t limit_tokens, size_t reserve_bytes = 0,
